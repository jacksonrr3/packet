...
,  };
		...
		std::cout << l2_udp->name() << ":\n" << 
			l2_udp->destination()->type() << " - " << l2_udp->destination()->to_string() << " " <<
			l2_udp->source()->type() << " - " << l2_udp->source()->to_string() << std::endl;
		auto l3_udp = p_udp.l3();
		std::cout << l3_udp->name() << ":\n" <<
			l3_udp->destination()->type() << " - " << l3_udp->destination()->to_string() << " " <<
			l3_udp->source()->type() << " - " << l3_udp->source()->to_string() << std::endl;
		auto l4_udp = p_udp.l4();
		std::cout << l4_udp->name() << ":\n" <<
			l4_udp->destination()->type() << " - " << l4_udp->destination()->to_string() << " " <<
			l4_udp->source()->type() << " - " << l4_udp->source()->to_string() << std::endl;

		Packet p_tcp;
		p_tcp.parse(vec_tcp.data(), vec_tcp.size());
		auto l2_tcp = p_tcp.l2();
		std::cout << l2_tcp->name() << ":\n" <<
			l2_tcp->destination()->type() << " - " << l2_tcp->destination()->to_string() << " " <<
			l2_tcp->source()->type() << " - " << l2_tcp->source()->to_string() << std::endl;
		auto l3_tcp = p_tcp.l3();
		std::cout << l3_tcp->name() << ":\n" <<
			l3_tcp->destination()->type() << " - " << l3_tcp->destination()->to_string() << " " <<
			l3_tcp->source()->type() << " - " << l3_tcp->source()->to_string() << std::endl;
		auto l4_tcp = p_tcp.l4();
		std::cout << l4_tcp->name() << ":\n" <<
			l4_tcp->destination()->type() << " - " << l4_tcp->destination()->to_string() << " " <<
			l4_tcp->source()->type() << " - " << l4_tcp->source()->to_string() << std::endl;
	}

	catch (std::exception & exception)
	{
		std::cerr << "Some other std::exception occurred (" << exception.what() << "), 0n";
	}
	return 0;
}
