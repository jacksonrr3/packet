#define BOOST_TEST_MODULE packet_test_module
#include <vector>
#include "packet.h"
#include <boost/test/unit_test.hpp>

BOOST_AUTO_TEST_SUITE(packet_test_suite)

BOOST_AUTO_TEST_CASE(packet_test_udp)
{

	std::vector<unsigned char> vec_udp = {  };


	Packet p_udp;
	p_udp.parse(vec_udp.data(), vec_udp.size());
	auto l2_udp = std::dynamic_pointer_cast<L2ProtoEthernet>(p_udp.l2());
	auto l3_udp = std::dynamic_pointer_cast<L3ProtoIPv4>(p_udp.l3());
	auto l4_udp = std::dynamic_pointer_cast<L4ProtoUDP>(p_udp.l4());

	BOOST_REQUIRE_EQUAL(l2_udp->name(), "Ethernet_version_2");
	BOOST_REQUIRE_EQUAL(l2_udp->destination_mac()->to_string(), "e8.5a.a7.20.10.2");
	BOOST_REQUIRE_EQUAL(l2_udp->source_mac()->to_string(), "18.31.bf.c.20.95");

	BOOST_REQUIRE_EQUAL(l3_udp->name(), "Internet_Protocol_version_4");
	BOOST_REQUIRE_EQUAL(l3_udp->destination_ip()->to_string(), "64.233.165.95");
	BOOST_REQUIRE_EQUAL(l3_udp->source_ip()->to_string(), "192.168.10.2");

	BOOST_REQUIRE_EQUAL(l4_udp->name(), "User_Datagram_Protocol");
	BOOST_REQUIRE_EQUAL(l4_udp->destination_port()->to_string(), "443");
	BOOST_REQUIRE_EQUAL(l4_udp->source_port()->to_string(), "55062");
		
}

BOOST_AUTO_TEST_CASE(packet_test_tcp)
{
	std::vector<unsigned char> vec_tcp = { 0xe8, 0x5a, 0xa7, 0x20, 0x10, 0x02, 0x18, 0x31, 0xbf, 0x0c, 0x20, 0x95, 0x08, 0x00, 0x45, 0x00, 0x00, 0x34,
		0x7b, 0xd4, 0x40, 0x00, 0x80, 0x06, 0x00, 0x00, 0xc0, 0xa8, 0x0a, 0x02, 0x0d, 0x21, 0xf2, 0xcc, 0xef, 0x84, 0x01, 0xbb, 0x03, 0x3a, 0xde, 0x04, 0x00,
		0x00, 0x00, 0x00, 0x80, 0x02, 0xfa, 0xf0, 0xca, 0xbe, 0x00, 0x00, 0x02, 0x04, 0x05, 0xb4, 0x01, 0x03, 0x03, 0x08, 0x01, 0x01, 0x04, 0x02 };

	Packet p_tcp;
	p_tcp.parse(vec_tcp.data(), vec_tcp.size());
	auto l2_tcp = std::dynamic_pointer_cast<L2ProtoEthernet>(p_tcp.l2());
	auto l3_tcp = std::dynamic_pointer_cast<L3ProtoIPv4>(p_tcp.l3());
	auto l4_tcp = std::dynamic_pointer_cast<L4ProtoTCP>(p_tcp.l4());

	BOOST_REQUIRE_EQUAL(l2_tcp->name(), "Ethernet_version_2");
	BOOST_REQUIRE_EQUAL(l2_tcp->destination_mac()->to_string(), "e8.5a.a7.20.10.2");
	BOOST_REQUIRE_EQUAL(l2_tcp->source_mac()->to_string(), "18.31.bf.c.20.95");

	BOOST_REQUIRE_EQUAL(l3_tcp->name(), "Internet_Protocol_version_4");
	BOOST_REQUIRE_EQUAL(l3_tcp->destination_ip()->to_string(), "13.33.242.204");
	BOOST_REQUIRE_EQUAL(l3_tcp->source_ip()->to_string(), "192.168.10.2");

	BOOST_REQUIRE_EQUAL(l4_tcp->name(), "Transmission_Control_Protocol");
	BOOST_REQUIRE_EQUAL(l4_tcp->destination_port()->to_string(), "443");
	BOOST_REQUIRE_EQUAL(l4_tcp->source_port()->to_string(), "61316");

}

BOOST_AUTO_TEST_SUITE_END()
